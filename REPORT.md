...
Depth was prioritized: one complete workflow, two tenant variants, and controlled faults. Metadata is currently in-memory rather than PostgreSQL. Continuous screencasting, broad co-browsing gestures, and fresh-state resume validation remain; the local operator console uses an intentionally bounded HTTP frame/input fallback. A paid genuine OpenAI evidence run requires a runtime key; the bounded provider path is implemented and tested, while replay needs no credential. See [Decisions and Cuts](docs/12-decisions-and-cuts.md).
