...
| Execution console | Launch and watch discovery/replay; inspect transient replay history; operate blocked live sessions | Persist raw screen recordings or bypass control leases |
...
| Discovery engine | Produce a verified draft, stop with failure, or suspend a blocked loop for same-session human correction |
