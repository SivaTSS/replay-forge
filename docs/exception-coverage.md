...
| Preserve compact text enclosures for click disambiguation | Require every button to exceed a page-wide median-font height | A current-frame component that fully encloses a label can distinguish it from repeated plain text; enclosing several matches or finding several bounded labels remains ambiguous. This does not classify the component as an editable field |
| Confirm competing labels in local image regions | Trust coarse component boxes or increase whole-frame analysis cost | Thin borders can disappear in bounded whole-frame analysis. OCR labels and component hints define local searches; complete borders must enclose the label without touching the crop boundary. Every competing match is checked, so a second button restores ambiguity. Surface-relative area limits, edge thresholds, confidence and deadlines remain unchanged |
| Structural step IDs | Generate control-flow addresses from model-authored target prose | Sequential action IDs contain no UI/customer text and remain stable when descriptions change; descriptions and executable targets still pass the privacy guard |
| Consistent privacy rules for typed output references | Substring-scan redaction-map keys as free text | Schema-validated output keys follow the same rule as checkpoint references; whole-symbol captured copies and invocation collisions still fail, and policy values remain scanned |
